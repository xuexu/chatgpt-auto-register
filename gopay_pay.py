...
class GoPayPayment:
    ...
    def _midtrans_post(self, path: str, body: dict, timeout: int = 15) -> dict:
        url = f"{MIDTRANS_BASE}{path}"
        r = self._session.post(url, headers=self._headers, data=json.dumps(body), timeout=timeout)
    ...
    def _midtrans_delete(self, path: str, timeout: int = 15) -> dict:
        url = f"{MIDTRANS_BASE}{path}"
        r = self._session.delete(url, headers=self._headers, timeout=timeout)
    ...
    def _gwa_post(self, path: str, body: dict, timeout: int = 15) -> dict:
        ...
        r = self._session.post(url, headers=headers, data=json.dumps(body), timeout=timeout)
    ...
    def _pin_verify(self, challenge_id: str, pin: str, client_id: str) -> str:
        ...
        r = self._session.post(url, headers=headers, data=json.dumps(body), timeout=15)
