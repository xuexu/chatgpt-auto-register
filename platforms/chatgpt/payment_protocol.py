...
from . import paypal_fraudnet
from . import paypal_http
from . import stripe_http

logger = logging.getLogger(__name__)


# ----- SMS relay code polling (inlined from payment.py) -----------------------
...
def _fetch_ctf_relay_code(
    *,
    ...
    initial_burst_attempts: int = 0,
    log=None,
    cancel_check=None,
    single_attempt: bool = False,
    excluded_pins=None,
) -> str:
    """轮询短信中继 URL，提取 6 位验证码。

    Args:
        url: 中继 HTTP GET URL
        timeout_seconds: 最长轮询时间
        poll_interval_seconds: 轮询间隔
        log: logging callback
        cancel_check: 取消检查回调
        single_attempt: 只试一次
        excluded_pins: 需要排除的旧验证码集合
    ...
    excluded = excluded_pins or set()
    deadline = _time.time() + max(1, timeout_seconds)

    if initial_burst_attempts > 0:
        for i in range(initial_burst_attempts):
                ...
                if r.status_code == 200:
                    try:
                        ...
                        if isinstance(data, list):
                            for item in data:
                                code = str(item).strip()
                                if _re.fullmatch(r"\d{4,8}", code) and code not in excluded:
                                    return code
                        elif isinstance(data, str):
                            match = _re.search(r"\b\d{4,8}\b", data)
                            if match and match.group(0) not in excluded:
                                return match.group(0)
                    except Exception:
                        match = _re.search(r"\b\d{4,8}\b", r.text)
                        if match and match.group(0) not in excluded:
                            return match.group(0)
            except Exception as exc:
                log_fn(f"relay poll burst[{i}] failed: {exc}")
            _time.sleep(1)
        if single_attempt:
            return ""
                ...
                try:
                    ...
                    if isinstance(data, list):
                        for item in data:
                            code = str(item).strip()
                            if _re.fullmatch(r"\d{4,8}", code) and code not in excluded:
                                log_fn(f"relay got code: {code}")
                                return code
                    elif isinstance(data, str):
                        match = _re.search(r"\b\d{4,8}\b", data)
                        if match and match.group(0) not in excluded:
                            log_fn(f"relay got code: {match.group(0)}")
                            return match.group(0)
                except Exception:
                    match = _re.search(r"\b\d{4,8}\b", r.text)
                    if match and match.group(0) not in excluded:
                        log_fn(f"relay got code: {match.group(0)}")
                        return match.group(0)
            log_fn(f"relay status={r.status_code} len={len(r.text)}")
        except Exception as exc:
            log_fn(f"relay poll failed: {exc}")

        if single_attempt:
            return ""
        _time.sleep(max(1, poll_interval_seconds))
...
_PAYPAL_SIGNUP_FAKE_PHONE = "5550100000"
...
class ProtoState:
    ...
    # ``{"phone": "5550000001", "phone_e164": "+15550000001",
    #    "relay_url": "https://example.invalid/api/text-relay/relay_token_1"}``。
...
def proto_stage_stripe_checkout(state: ProtoState) -> StageResult:
    """Stripe Checkout 协议化阶段：init → 提交税务地址 → 创建 PayPal PM → confirm。

    成功时把 ``setup_intent.next_action.redirect_to_url.url`` 作为下一段 stage 的
    ``next_url``（即 PayPal 跳转入口），并将 ``payment_method_id``、
    ``init_checksum`` 等关键字段写入 ``StageResult.detail``，便于后续 stage 复用。
    """
    ...
    # 只列了 ``[card]`` 而我们传 ``payment_method_id=pm_paypal``，服务端就报
    ...
    tax_expected_amount = stripe_http.extract_expected_amount(tax_resp)
    ...
    state.log(f"[{stage}] 创建 PayPal PaymentMethod (email={state.email})")
    try:
        ...
        return StageResult(ok=False, stage=stage, error=f"payment_methods 失败: {exc}", fallback_recommended=True)
    pm_id = str(pm_resp.get("id") or "")
    if not pm_id:
        return StageResult(
            ok=False,
            stage=stage,
            error="创建 PayPal PaymentMethod 未返回 id",
            fallback_recommended=True,
            detail={"payment_method_resp_keys": list(pm_resp.keys())[:20]},
        )
    state.log(f"[{stage}] payment_method_id={pm_id}")

    state.raise_if_cancelled()
    state.log(f"[{stage}] /confirm")
    try:
        confirm_resp = stripe_http.stripe_confirm_paypal(
            state.session,
            cs_id=cs_id,
        ...
        return StageResult(ok=False, stage=stage, error=f"/confirm 失败: {exc}", fallback_recommended=True)

    try:
        redirect_url, return_url = stripe_http.extract_paypal_redirect_url(confirm_resp)
    except ValueError as exc:
        return StageResult(ok=False, stage=stage, error=str(exc), fallback_recommended=True)
...
def _local_phone_from_e164(phone_e164: str, *, default_country_digit: str = "1") -> str:
    ...
    例如:
    - ``+15550000001`` → ``5550000001``（北美 +1 之后是 10 位本地号）
    - ``15550000001`` （无 +）→ ``5550000001``（按 default_country_digit 剥）
...
def _run_paypal_otp_subchain(
    *,
    ...
    # NOTE: 本项目独立部署，payment.py 缺失，使用内联 _fetch_ctf_relay_code
    _payment_module = None
    ...
    if email_for_otp:
        try:
            # 优先使用从 PayPal 落地页 HTML 提取的真实 token，fallback 到随机生成
            ctx_csrf = state.checkout_context.get("otp_csrf_nonce") or ""
            ctx_id = state.checkout_context.get("otp_ctx_id") or ""
            if ctx_csrf and ctx_id:
                challenge_csrf, challenge_ctx = ctx_csrf, ctx_id
                state.log("[paypal_signup/otp] 使用落地页提取的真实 csrfNonce/ctxId")
            else:
                challenge_csrf, challenge_ctx = paypal_http.generate_otp_challenge_tokens()
                state.log("[paypal_signup/otp] 落地页未提取到 token，使用随机占位")
    ...
    #     所有 6 位数字都记下来。relay 服务（如 example.invalid）可能
    ...
    try:
        ...
        baseline_pin = _fetch_ctf_relay_code(
            url=relay_url,
            timeout_seconds=10,
